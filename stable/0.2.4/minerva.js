...
var mvHeader = function(minerva)
{
   return {
      ...
      template:      '<div class="mv-header"><div class="mv-title">{{mvTitle}}</div><div class="mv-title-right"><ng-transclude></ng-transclude></div></div>'
   };
};
...
var mvBody = function(minerva)
{
   return {
      ...
      template:      '<div class="mv-body"><div class="mv-widget-container g{{mvCols}}r"><ng-transclude></ng-transclude></div></div>'
   };
};
...
var mvGroup = function(minerva)
{
   return {
      ...
      template:      '<div class="gr-{{mvSize}} break" ng-style="{{mvStyle}}"><ng-transclude></ng-transclude></div>'
   };
};
...
var mvIdBox = function(minerva)
{
   return {
      ...
      template:      '<div class="widget g-{{mvSize}}" id="{{mvId}}"><div class="w-head lightbox-head">{{mvTitle}}</div><div class="w-body lightbox-body" id="{{mvBodyId}}"><ng-transclude></ng-transclude></div></div>'
   };
};
...
var mvTableList = function(minerva)
{
   return {
      ...
      template:      '<div class="widget g-{{mvSize}}" id="{{mvId}}"><div class="w-head lightbox-head">{{mvTitle}}</div><div class="w-body lightbox-body"><div class="w-table-list" ng-style="{{mvStyle}}"><table ng-repeat="stats in mvData"><tr><th class="mv-table-list-title" colspan="2">{{stats.title}}</th></tr><tr ng-repeat="stat in stats.data"><th>&emsp;<span class="{{stat.color}} || w-c{{$index}}">&emsp;</span> {{stat.title}}</th><td>{{stat.number}}</td></tr><tr><td>&nbsp;</td></tr></table></div></div></div>'
   };
};
   ...
   return { 
      ...
      link: function(scope, element, attrs, ctrl, transclude)
      {
                     ...
                     case 'map': //TODO: fix
                        switch (o.types.map.mapfile)
                        {
                           case 'afg':
                              ...
                              break;
                           default:
                              title = idToName(d.id) || 'Name';
                              tipData = tipformat(map.get(d.id));
                              if (map.get(d.id) == undefined) return '<strong>' + title + '</strong>';
                              break;
                        }
                  }               
                  return '<strong>' + title + ':</strong> <span style="color:' + o.tooltip.color + '">' + o.tooltip.prefix + tipData + o.tooltip.suffix + '</span>'; 
               });
                  ...
                  d3.json(mapFile, function(error, json)
                  {
                     ...
                     if (data.points != undefined && o.types.map.showPoints == true)
                     {
                        ...
                        svg.selectAll('path')
                         ...
                         .enter().append('path')
                           .datum(function(d) {
                              return {type: 'Point', coordinates: [d.Lon, d.Lat], radius: d.Magnitude, color: d.Color};
                           })
         ...
         function clippedSatellite() {
            ...
            var clip = d3.geo.projection(function(lambda, phi) {
               return [lambda, -phi];
               ...
               var pstream = projectionStream.call(projection, stream),
                  ...
                  clipStream = alpha ? clip.stream({
                     point: function(lambda, phi) {
                        var point = tiltRotate.invert([lambda, phi]);
