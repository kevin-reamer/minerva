...
*/
var mvAnyNumLink =  function(minerva)
{
   return {
      restrict:      'E',
      transclude:    true,
      ...
      templateUrl:   minerva.templatesPath() + 'mv-any-num-link.txt',
...
/*
   ...
   Parameters: 
   		...
   		mv-Style: Any custom style for the table
*/
var mvTableLink = function(minerva)
{
   return {
      restrict:      'E',
      replace:       true,
      ...
      templateUrl:   minerva.templatesPath() + 'mv-table-link.txt'
   };
};
            ...
            switch (o.chart.type)
            {
               case 'pie':
                  ...
                  g.append("path")
                     ...
                     .on('click', function(d) { tip.hide(); scope.$parent.openProduct(d); })
               ...
               case 'bubble':
                  ...
                  circles.enter().append('circle')
                     ...
                     .on('click', function() { tip.hide(); scope.$parent.openProduct(); })
                     .on('mouseover', tip.show)
                     .on('mouseout', tip.hide);
                  ...
                  switch (o.chart.type)
                  {
                     ...
                     case 'hbar':
                        ...
                        var layers = stack(d3.range(n).map(function(d) { 
                           var a = [];
                           for (var i = 0; i < m; ++i) {
                              a[i] = {x: labels[i], y: data[i]['value' + (d+1)], color: data[i]['color' + (d+1)], label: data[i]['label' + (d+1)], children: data[i].children};  
                           }
                           return a;
                        }));
...
angular.module('mvMinerva', [])
   ...
   .directive('mvAnyNumLink', mvAnyNumLink)
